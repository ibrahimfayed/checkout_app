...
try {
  ...
  Future initPaymentSheet({required InitPaymentSheetInputModel initPaymentSheetInputModel})async{//code for init payment sheet and there is some parameters required
...
try {
  ...
  ({required String customerId})async{//i use customerId not the model because it only one parameter
try {
  var response = await apiService.post(
  ...
  var ephemeralKeyModel = EphemeralKeyModel.fromJson(response.data);//may cause an error and i solved it in notebook and vidio of integrate checkout cubit
